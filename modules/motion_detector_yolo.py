...
class TrackedVehicle:
    ...
    def classify_event(self):
        """
        Classifies road event from smoothed Y-position history.
        Uses consecutive differences NOT max-min range.
        """
        now = time.time()
        if self.held_event!='normal' and now<self.held_until:
            return self.held_event, self.held_conf, self.held_desc
 
        if len(self.y_smooth)<6:
            return 'normal',0.0,'Collecting data'
 
        ys = list(self.y_smooth)   # smoothed positions
        ws = list(self.w_buf)
 
        # Consecutive differences on SMOOTHED data
        ...
        # ── POTHOLE ──
        # Single step > threshold on smoothed data
        # Smoothing already removes YOLO noise
        if max_step > self.POTHOLE_STEP_PX:
            large = sum(1 for s in y_abs if s > self.POTHOLE_STEP_PX*0.5)
            if large <= 4:   # short duration = pothole
                conf = min(1.0, max_step/30.0)
                self._hold('pothole', conf,
                    f'Impact {max_step:.0f}px detected')
                return self.held_event,self.held_conf,self.held_desc
 
        # ── SPEED HUMP ──
        # Sustained moderate movement across many frames
        if mean_step > self.HUMP_STEP_PX:
            sustained = sum(1 for s in y_abs if s>self.HUMP_STEP_PX*0.6)
            if sustained >= self.HUMP_MIN_FRAMES:
                conf = min(1.0, sustained/12.0)
                self._hold('speed_hump', conf,
                    f'Sustained {mean_step:.1f}px/f hump')
                return self.held_event,self.held_conf,self.held_desc
 
        # ── BRAKING ──
        # Box width growing = car approaching fast
        if mean_w_grow > self.BRAKE_GROW_PX and mean_step < 8:
            conf = min(1.0, mean_w_grow/12.0)
            self._hold('braking', conf,
                f'Approaching +{mean_w_grow:.1f}px/f')
            return self.held_event,self.held_conf,self.held_desc
...
class MotionDetectorYOLO:
    ...
    def detect(self, frame):
        ...
        # ── Find primary (largest = closest) ──
        self.primary=None
        best=0
        for vid,tv in self.vehicles.items():
            x1,y1,x2,y2=tv.box
            a=(x2-x1)*(y2-y1)
            if a>best: best=a; self.primary=tv
    ...
    def get_vehicle_count(self):
        return len(self.vehicles)   # live count — resets as vehicles leave
